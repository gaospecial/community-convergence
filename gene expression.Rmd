...
ck_plot <- function(ck){
  ...
  ggplot(df, aes(time, Description, size = GeneRatio)) +
    geom_point() +
...
```{r kegg-ora, fig.width=8, fig.asp=1.2, fig.cap="(ref:kegg-ora-cap) "}
plot_grid(p1,p2, rel_heights = c(1,0.4), ncol = 1, labels = "AUTO",align = "v")
```
...
gse_dotplot <- function(df){
  ggplot(df, aes(time, Description, color = enrichmentScore)) +
    geom_point() +
...
## Combine tree and GSEA result

There are too many pathway identifided by GSEA KEGG.
...
ecopath <- df1 %>% 
  ...
  pivot_wider(id_cols = Description,
              names_from = core_enrichment,
              values_from = value,
              values_fill = list(value = 0))

library(vegan)
matrix <- as.matrix(column_to_rownames(ecopath, var = "Description"))
...
```{r}
d1 <- df1 %>%
  dplyr::select(Description, setSize, enrichmentScore, ratio, time, type) %>%
  rename(id = "Description") %>%
  mutate(time = as.numeric(time))

peco <- p + geom_facet(panel = "1:1000",
               data=filter(d1, ratio == "1:1000"), 
               geom=geom_point, 
               mapping = aes(x=time,color=type,size=setSize)) +
  geom_facet(panel = "1:1",
             data=filter(d1, ratio == "1:1"), 
             geom=geom_point, 
             mapping = aes(x=time,color=type,size=setSize)) +
  geom_facet(panel = "1000:1",
             data=filter(d1, ratio == "1000:1"), 
             geom=geom_point, 
             mapping = aes(x=time,color=type,size=setSize)) 
  
peco
# TODO: 调整 facet 的 width
```
...
library(vegan)
matrix <- as.matrix(column_to_rownames(ppupath, var = "Description"))
dist <- vegdist(matrix)

library(ape)
library(ggtree)
tree <- nj(dist)
ggtree(tree) + geom_tiplab() + xlim(0,1)
```
